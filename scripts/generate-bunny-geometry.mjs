...
const svg = readFileSync('public/pseudobun.svg', 'utf8');
...
// ---- coordinate mapping ------------------------------------------------------
// Artwork spans x 104..408, y 15..496 inside a 512 viewBox. Scale by height so
// the head fills the same ~2.6 model units the camera is framed for.
const SCALE = 2.6 / (496 - 15);
const cX = 256; // axis of symmetry
const cY = 330; // vertical model origin sits on the face
...
// Facets are polygons of up to 6 points and not all convex, so a plain fan can
// emit triangles outside the facet. Ear-clip instead.
function earClip(ks) {
  const idx = [...new Set(ks)];
  if (idx.length < 3) return [];
  let area = 0;
  for (let i = 0; i < idx.length; i++) {
    const a = uniq.get(idx[i]);
    const b = uniq.get(idx[(i + 1) % idx.length]);
    area += a.x * b.y - b.x * a.y;
  }
  const sign = Math.sign(area) || 1;
  const cross = (ka, kb, kc) => svgArea(ka, kb, kc) * sign;
  const inside = (kp, ka, kb, kc) =>
    cross(ka, kb, kp) >= 0 && cross(kb, kc, kp) >= 0 && cross(kc, ka, kp) >= 0;
  const out = [];
  while (idx.length > 3) {
    let clipped = false;
    for (let i = 0; i < idx.length; i++) {
      ...
      if (cr < 0) continue; // reflex corner
      if (cr > 0 && idx.some((k) => k !== a && k !== b && k !== c && inside(k, a, b, c))) continue;
      if (cr > 0) out.push([a, b, c]);
      idx.splice(i, 1); // collinear corners are dropped without a triangle
      clipped = true;
      break;
    }
    if (!clipped) break;
...
const NOSE_X = 256;
const NOSE_Y = 440; // svg coords of the nose tip
const MUZZLE = 0.24;
const MUZZLE_R = 60;
const EYE_MAX_Y = 390; // dark facets above this line are eyes -> recess
...
const out = `// AUTO-GENERATED by scripts/generate-bunny-geometry.mjs from public/pseudobun.svg.
